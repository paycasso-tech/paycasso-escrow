  ...
  return (
    ...
    >
      {text.split('').map((char, i) => (
        <span key={i} style={{ opacity: i < visibleCount ? 1 : 0, transition: 'opacity 0.2s' }}>{char}</span>
      ))}
    </div>
  );
}
...
export default function HeroSection() {
  return (
    <section className="w-full flex flex-col items-center font-poppins">
      ...
      <SectionLabel className="mt-12 mb-8 text-xl">Shaping the future of finance together</SectionLabel>
    </section>
  );
} 